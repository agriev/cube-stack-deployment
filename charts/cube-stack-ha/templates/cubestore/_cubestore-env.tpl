...
The DNS name is resolved via the dedicated headless service
`<rel>-cubestore-router-headless` (clusterIP: None,
publishNotReadyAddresses: true). Kubernetes emits per-pod A records
at `<podname>.<headless>.<ns>.svc.cluster.local`. The headless
service is purpose-built for Raft peer discovery; client traffic
keeps going to the regular ClusterIP `router-service`.
*/}}
{{- define "cubeStack.cubestore.raftPeers" -}}
{{- $base := include "cubeStack.cubestore.router.fullname" . -}}
{{- $headless := include "cubeStack.cubestore.router.headless" . -}}
